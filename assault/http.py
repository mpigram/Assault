...
async def distribute_work(url, requests, concurrancy, results):
    queue = asyncio.Queue()
    # Add an item to the queue for each request we want to make
    for _ in range(requests):
        queue.put_nowait(url)
    # Create wokers to match the concurrency
    tasks = []
    for i in range(concurrancy):
...
def assault(url, requests, concurrancy):
    results = []
    total_time = asyncio.run(distribute_work(url, requests, concurrancy, results))
